...
class DominatingBoundPairCalculatorTest extends FunSuite with Matchers {
  ...
  private val dominatingBoundPairCalculator = new DominatingBoundPairCalculator(
    cellsPerDimension
  )
  ...
  test(
    ...
    val actual = dominatingBoundPairCalculator
      .calculate(cell, pointCount, neighboringCellDominatingBounds)
  ...
  test(
    ...
    val pointCount = 9
    val neighboringCellDominatingBounds =
      Map(Seq(2, 2, 2).toCell -> BoundPair(0, 10))
